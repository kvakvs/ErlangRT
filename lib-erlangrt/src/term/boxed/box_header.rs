...
impl BoxHeader {
            ...
            TraitType: TBoxed + 'static,
    {
        ...
        let trait_ptr = ptr::null_mut::<TraitType>() as *mut dyn TBoxed;
        // let trait_obj: std::raw::TraitObject = unsafe { core::mem::transmute(trait_ptr) };
        let (_null_obj, dyn_metadata) = trait_ptr.to_raw_parts();
        Self::create_bare(header_word, dyn_metadata)
    }

    #[cfg(debug_assertions)]
    pub const fn create_bare(header_word: usize, dyn_metadata: DynMetadata<dyn TBoxed>) -> Self {
        BoxHeader {
            header_word,
            guard_word: GUARD_WORD_VALUE,
            trait_vtab: dyn_metadata,
        }
    }
    ...
    #[inline]
    pub fn get_trait_ptr(&self) -> *const dyn TBoxed {
        self.ensure_valid();
        // let trait_obj1 = core::raw::TraitObject {
        //   data: self as *const Self as *mut (),
        //   vtable: self.trait_vtab,
        // };
        let trait_obj = std::ptr::from_raw_parts::<dyn TBoxed>(
            self as *const Self as *mut (), self.trait_vtab);
        unsafe { core::mem::transmute(trait_obj) }
    }

    #[inline]
    #[allow(dead_code)]
    pub fn get_trait_ptr_mut(&mut self) -> *mut dyn TBoxed {
        self.ensure_valid();
        ...
        // };
        let trait_obj = std::ptr::from_raw_parts::<dyn TBoxed>(
            self as *mut Self as *mut (), self.trait_vtab);
        unsafe { core::mem::transmute(trait_obj) }
    }
